...
## How do I hand in these problems for the 11:59 a.m. ET, November 25 deadline?

Your complete .Rmd file that you create for this problem set AND the resulting .pdf (i.e., the one you 'Knit to PDF' from your .Rmd file) must be uploaded into a Quercus assignment (link:[https://q.utoronto.ca/courses/235890/assignments/742955](https://q.utoronto.ca/courses/235890/assignments/742955)) by 11:59 a.m. ET, on November 25. Late problem sets or problems submitted outside of Quercus (e.g., by email) are *not* accepted.
...
### (a) Begin by creating a new variable called `life_exp_category` which takes the value "Good" for countries with a life expectancy higher than 60 years, and "Poor" otherwise. 
...
For the tree built in part (b), ....

```{r}
### Tree from (c)

```
For the tree built in part (c)...


### (e) Fill in the following table using the tree you constructed in part (c).  Does the fact that some of the values are missing (NA) prevent you from making predictions for the life expectancy category for these observations?  (I.e., fill in the ????s)

|               | `logGDP` | `social_support` | `freedom` | `generosity` | Predicted life expectancy category |
|---------------|----------|------------------|-----------|--------------|------------------------------------|
| Obs 1         | 9.67     | 0.74             | NA        | -0.25        | **????**                           |
| Obs 2         | 8.11      | 0.84             | 0.80      | -0.219       | **????**                           |
| Obs 3         | 8.49     | 0.88             | 0.77      | 0.1          | **????**                           |



### (f) In most cases, two classification trees will make different predictions for some new observations. Using the classification trees you built in parts (b) and (c), fill in the table below with values which would lead the specified predictions. (I.e., fill in the ??s)
...
### The diagram above is the geometric interpretation of a classification tree to predict drug effectiveness based on two predictors, x1 and x2. What is the predicted class of each node? (I.e., fill in the ????s)
